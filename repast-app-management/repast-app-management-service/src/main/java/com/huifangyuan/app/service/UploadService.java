...
import com.huifangyuan.app.properties.FtpProperties;
import com.huifangyuan.app.utils.FileNameUtil;
import com.huifangyuan.app.utils.FtpUtil;
...
public class UploadService {
    ...
    public String uploadHead(MultipartFile file,String token,MemberCommentService memberCommentService) {
        if(null != token || !"".equals(token)){
            System.out.println("有值额");
            Member member = memberCommentService.memberToken(token);
            System.out.println("用户的信息为"+member);
            if(null == member){
            ...
            try {
                ...
                String newFileName = FileNameUtil.getFileName(member.getId());
                // 3.获取原始文件的后缀名
